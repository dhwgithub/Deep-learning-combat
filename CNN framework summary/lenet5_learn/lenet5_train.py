...
import cnn_learn.lenet5_learn.lenet5_inference as my_lenet5
...
def train(mnist):
    ...
    x = tf.placeholder(tf.float32, [BATCH_SIZE,
                                    ...
                                    my_lenet5.IMAGE_SIZE,
                                    my_lenet5.NUM_CHANNELS], name='x-input1')
    y_ = tf.placeholder(tf.float32, [None, my_lenet5.OUTPUT_NODE], name='y-input')

    regularizer = tf.contrib.layers.l2_regularizer(REGULARAZTION_RATE)  # 返回一个可以计算l2正则化项的函数

    # 前向传播过程
    y = my_lenet5.inference(x, True, regularizer)
...
'''
****************开始训练************************
After 1 training step (s) , loss on training batch is 2.37189.
After 1001 training step (s) , loss on training batch is 0.825284.
After 2001 training step (s) , loss on training batch is 0.679484.
After 3001 training step (s) , loss on training batch is 0.298936.
After 4001 training step (s) , loss on training batch is 0.325703.
After 5001 training step (s) , loss on training batch is 0.315715.
After 6001 training step (s) , loss on training batch is 0.396986.
After 7001 training step (s) , loss on training batch is 0.225926.
*******************训练结束****************************
'''