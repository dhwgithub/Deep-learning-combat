...
import cnn_learn.make_my_dataset.make_own_data as read_dataset
...
'''
解析出tfrecords文件，其中label标签需要进行one_hot处理
'''
def get_datasets(train_tfrecords_path, test_tfrecords_path, batch_size=10):
    img, label = read_dataset.read_and_decode(train_tfrecords_path)
    img_test, label_test = read_dataset.read_and_decode(test_tfrecords_path)

    # 使用train.shuffle_batch可以随机打乱输入
    img_batch, label_batch = tf.train.shuffle_batch([img, label],
                                                    batch_size=batch_size, capacity=2000,
                                                    min_after_dequeue=1000, name='train_dataset')
    img_test, label_test = tf.train.shuffle_batch([img_test, label_test],
                                                  batch_size=batch_size, capacity=2000,
                                                  min_after_dequeue=1000, name='test_dataset')
...
if __name__ == '__main__':
    ...
    img_batch, label_batch, img_test, label_test = get_datasets(train_tfrecords_path, test_tfrecords_path)
    ...
    with tf.Session() as sess:
        ...
        train_sum = 261
        batch_idxs = int(train_sum / batch_size)  # 需要训练几批次
