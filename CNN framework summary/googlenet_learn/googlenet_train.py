...
import cnn_learn.googlenet_learn.googlenet_inference as inference

####配置实现指数衰减学习率的相关参数######
BATCH_SIZE = 25  # 一个训练batch中的训练数据个数。数字越小时，训练过程越接近随机梯度下降；数字越大时，训练越接近梯度下降
...
'''
启动可视化界面命令：
tensorboard --logdir=E:\pycharm\tensorflow-learn\cnn_learn\googlenet_learn\logs
'''
'''
****************开始训练************************
After 1 training step (s) , loss on training batch is 2.87147.
After 1001 training step (s) , loss on training batch is 2.22605.
After 2001 training step (s) , loss on training batch is 2.11596.
After 3001 training step (s) , loss on training batch is 1.44359.
After 4001 training step (s) , loss on training batch is 0.797844.
After 5001 training step (s) , loss on training batch is 0.551648.
After 6001 training step (s) , loss on training batch is 0.320496.
After 7001 training step (s) , loss on training batch is 0.4264.
*******************训练结束****************************
'''